...
class WP_Job_Board_Pro_Post_Type_Partner {

	public static $prefix = WP_JOB_BOARD_PRO_PARTNER_PREFIX;
	...
	public static function init() {
	  	...
	  	// add_filter( 'manage_edit-partner_columns', array( __CLASS__, 'custom_columns' ) );
		// add_action( 'manage_partner_posts_custom_column', array( __CLASS__, 'custom_columns_manage' ) );

		// add_action( 'restrict_manage_posts', array( __CLASS__, 'filter_partner_by_type' ) );
		...
		// add_action( 'cmb2_save_field_'.self::$prefix . 'partners', array( __CLASS__, 'save_field_partners' ), 10, 3 );
	}

	public static function register_post_type() {
		$singular = __( 'Partner', 'wp-job-board-pro' );
		$plural   = __( 'Partners', 'wp-job-board-pro' );
		...
		$has_archive = true;
		$partner_archive = get_option('wp_job_board_pro_partner_archive_slug');
		if ( $partner_archive ) {
			$has_archive = $partner_archive;
		}
		$rewrite_slug = get_option('wp_job_board_pro_partner_base_slug');
		if ( empty($rewrite_slug) ) {
			$rewrite_slug = _x( 'partner', 'Partner slug - resave permalinks after changing this', 'wp-job-board-pro' );
		}
		$rewrite = array(
			'slug'       => $rewrite_slug,
			'with_front' => false
	...
	public static function process_denied_to_publish($post) {
		if ( $post->post_type === 'partner' ) {
			$user_id = WP_Job_Board_Pro_User::get_user_by_partner_id($post->ID);
	...
	public static function save_field_partners($updated, $action, $obj) {
		if ( $action != 'disabled' ) {
			$key = self::$prefix.'partners';
	...
	public static function metaboxes() {
		...
		if ( $pagenow == 'post.php' || $pagenow == 'post-new.php' ) {
            BugFu::log('metaboxes');
			do_action('wp-job-board-pro-partner-fields-admin');
		}
	}
...
WP_Job_Board_Pro_Post_Type_Partner::init();