...
class My_Custom_Job_Board_User extends WP_Job_Board_Pro_User {
	
	public static function init() {
		BugFu::log('My_Custom_Job_Board_User::init()');
	...
	public static function registration_save($user_id) {
        ...
        if ( $user_role == 'wp_job_board_pro_partner' ) {
            ...
            $post_args = apply_filters('wp-job-board-pro-create-partner-post-args', $post_args, $user_obj);

            // Insert the post into the database
            $partner_id = wp_insert_post($post_args);
            
            $post_id = $partner_id;

            update_post_meta($partner_id, $prefix . 'user_id', $user_id);
            update_post_meta($partner_id, $prefix . 'display_name', $display_name);
            update_post_meta($partner_id, $prefix . 'email', $user_obj->user_email);
            update_post_meta($partner_id, $prefix . 'show_profile', 'show');
            ...
            if ( wp_job_board_pro_get_option('candidates_requires_approval', 'auto') != 'auto' && ($action == 'wp_job_board_pro_ajax_register' || $action == 'wp_job_board_pro_ajax_registernew') ) {
				...
				$email_key = apply_filters('wp_job_board_pro_email_key_partner_user_register_need_approve', 'user_register_need_approve');
            	...
            	$email_key = apply_filters('wp_job_board_pro_email_key_partner_user_register_auto_approve', 'user_register_auto_approve');
            ...
            update_post_meta($partner_id, 'post_date', strtotime(current_time('d-m-Y H:i:s')));

            if ( isset($_POST['phone']) ) {
	            update_post_meta($partner_id, $prefix . 'phone', $_POST['phone']);
	        }
            if (isset($_POST['partner_category'])) {
                $partner_category = sanitize_text_field($_POST['partner_category']);
                wp_set_post_terms($partner_id, array($partner_category), 'partner_category', false);
            }

            // custom fields saving
            do_action('wp_job_board_pro_partner_signup_custom_fields_save', $partner_id);
             
            do_action('wp_job_board_pro_signup_custom_fields_save', 'partner', $partner_id);
            
            $wp_job_board_pro_socials_register = false;
        }

        do_action('wp_job_board_pro_member_after_making_partner', $user_id, $user_role);
